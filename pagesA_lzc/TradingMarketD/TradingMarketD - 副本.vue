	...
	<view class="content">
		<view class="liu1" v-if="isLiu">
			<view class="flS dis_flex aic ju_b">
				<view class=""></view>
				<text class="title_text fwb">全部留言（{{shuju.message.count}}）</text>
			...
			<scroll-view scroll-y="true" style="height: 500upx;" @scrolltolower="gun2">
				<view class="sls" v-for="(item,index) in liuList.data" :key="index" @click="jump"
				...
				<view class="sls zanwu" v-if="shuju.message.list.length==0">
					暂无数据
				</view>
			</scroll-view>
		...
		<view class="jvsl">
			<view class="jvsl_sl dis_flex">
				...
				<view class="jvsl_slr bs">
					<view class="dis_flex">
						<view class="biao dis_flex ju_c aic">
							<text>{{shuju.info.pInfo.title}}</text>
						</view>
						<view class="jvsl_slr_title oh2">
							<text>{{shuju.info.title}}</text>
						</view>
					</view>
				</view>
			</view>
			<!-- 标签 -->
			<view class="myTag bs dis_flex aic">
		...
		<view class="dis_flex ju_b  aic info bs">
			<view class="imgI">
			...
			<view class="btn_right dis_flex aic ju_c" @click="jump"
				:data-url="`${Phomepage_url}?id=${shuju.userInfo.id}`">
				进入主页
			</view>
		</view>
		<!-- 商品详情 -->
			...
			<view class="" v-for="(item,index) in shuju.info.params_arr">
				...
				<view class="sls">
					...
					<view class="dis_flex ju_b" v-for="(item1,index1) in item.info" v-if="item1.type!=6">
						...
						<view style="width: 100%;"><text>{{item1.value.toString()}}</text></view>
					</view>
					<view class="dis_flex ju_b" v-for="(item1,index1) in item.info" v-if="item1.type==6">
						<text>{{item1.title}}：</text>
						<image style="width: 216upx;height: 216upx;"  :src="getimg(item2)" mode="aspectFill" v-for="item2,index in item1.value.split(',')"></image>
					</view>
					<!-- <view class="dis_flex ju_b">
						<text>类似群组：</text>
						<view style="width: 100%;"><text>{{shopDetail.qunList}}</text></view>
					</view>
					<view class="dis_flex ju_b">
						<text>是否独家：</text>
						<view style="width: 100%;"><text>{{shopDetail.isDu}}</text></view>
					</view>
					<view class="dis_flex ju_b">
						<text>适用项目：</text>
						<view style="width: 100%;"><text>{{shopDetail.Suitableproject}}</text></view>
					</view>
					<view class="dis_flex ju_b">
						<text>组合类型：</text>
						<view style="width: 100%;"><text>{{shopDetail.CombinationType}}</text></view>
					</view>
					<view class="dis_flex ju_b">
						<text>有效期至：</text>
						<view style="width: 100%;"><text>{{shopDetail.time}}</text></view>
					</view> -->
				</view>
			</view>
			<!-- <view class="flS dis_flex">
				...
				<text class="title_text fwb">商品详情</text>
			</view> -->
			<!-- <view class="sls">
				<view class="dis_flex ju_b">
					<text>商标分类：</text>
					<view style="width: 100%;"><text>{{shopDetail.type}}</text></view>
				</view>
				<view class="dis_flex ju_b">
					<text>类似群组：</text>
					<view style="width: 100%;"><text>{{shopDetail.qunList}}</text></view>
		...
		<!-- 详细描述 -->
		<!-- <view class="DetailedD">
			<view class="flS dis_flex aic">
				<image class="title_image" src="/static/images/lzc/fuIndex/biao1.png" mode="aspectFill">
				</image>
				<text class="title_text fwb">详细描述</text>
			</view>
			<view class="sls">
				<view class="dis_flex ju_b">
					<text>产品描述：</text>
					<view style="width: 100%;"><text>{{DetailedD.shopDetail}}</text></view>
				</view>
				<view class="dis_flex ju_b">
					<text>商品地区：</text>
					<view style="width: 100%;"><text>{{DetailedD.add}}</text></view>
				</view>
			</view>
		</view> -->
		<!-- 图片信息 -->
		<!-- <view class="pictureInfo">
			<view class="flS dis_flex aic">
				...
				<text class="title_text fwb">图片信息</text>
			</view>
			<view class="sls dis_flex ju_b">
				<image :src="getimg(img.img)" mode="aspectFill" v-for="img in imgs"></image>
			</view>
		</view> -->
		<!-- 留言 -->
		<view class="liu">
			...
			<!-- <view class="sls" v-for="(item,index) in shuju.message.list" :key="index" @click="jump"
				data-url="/pagesA_lzc/Phomepage/Phomepage">
				<view class="sls_fc bs dis_flex">
					<image :src="getimg(item.userInfo.img)" mode="aspectFill"></image>
					<text class="tel" style="margin: 0 45upx 0 10upx;">{{item.userInfo.phone}}</text>
					<text class="time">{{item.create_time}}</text>
			...
			<scroll-view scroll-y="true" style="height: 590upx;" @scrolltolower="gun1">
				<view class="sls" v-for="(item,index) in liuList.data" :key="index" @click="jump"
					data-url="/pagesA_lzc/Phomepage/Phomepage" v-if="shuju.message.list.length!=0">
					<view class="sls_fc bs dis_flex">
						<image :src="getimg(item.userInfo.img)" mode="aspectFill"></image>
		...
		<view class="foot dis_flex ju_b aic">
			<view class="footLeft dis_flex">
				<button class="dis_flex_c litem aic ju_c" @click="ShouClick">
					<text class="iconfont icon-Starmarker-b" v-if='isShou'></text>
					<text class="ltext" v-if='isShou'>收藏</text>
					<text class="iconfont icon-guanzhu" v-if='isShou==false'></text>
					<text class="ltext" v-if='isShou==false'>已收藏</text>
				</button>
				<button class="dis_flex_c litem aic ju_c" @click="liu">
					<text class="iconfont icon-pinglun"></text>
					<text class="ltext">留言</text>
				</button>
				<button class="dis_flex_c litem aic ju_c" @click="jump" data-url="/pagesA_lzc/kefu/kefu">
					<text class="iconfont icon-fuwudingdanxiangqing-kefu"></text>
					<text class="ltext">客服</text>
				</button>
			</view>
			<view class="footRight dis_flex ju_b">
				<view class="dis_flex ju_c aic" @click="jump" data-url="/pagesA_lzc/pay/pay">立即购买</view>
				<view class="dis_flex ju_c aic" @click="jump" data-url="/pagesA_lzc/kefu/kefu">联系卖家</view>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				shopDetail: {
					type: '25类-服装鞋帽',
					qunList: '2501，2502，2503，2504，2505，2506，2507，2508，2509，2510，2511',
					isDu: '是',
					Suitableproject: '上衣，非纸质围涎，游泳衣，雨衣，舞衣，鞋，帽，袜，手套（服装），围巾',
					CombinationType: '中文+拼音',
					time: '2030-08-27'
				},
				DetailedD: {
					shopDetail: '25类-服装鞋帽',
				...
				imgs: [{
						img: '/static/images/lzc/deatil/tu1.png'
					},
					{
						img: '/static/images/lzc/deatil/tu1.png'
					},
					{
						img: '/static/images/lzc/deatil/tu2.png'
					}
				],
				liuList: [{
		...
		methods: {
			...
			liuBtn(id) {
				api.P_post("transaction_list/message_add", {
					id: id,
					content: this.liuText,
					type: 1
				}).then((res) => {
					if (res.code == 1) {
						uni.showToast({
			...
			/* 获取数据 */
			getData(id) {
				api.P_post("transaction_list/detail", {
					id: id
				}).then((res) => {
					if (res.code == 1) {
						this.shuju = res.data;
			...
			/* 获取留言列表 */
			getLiu(limit) {
				api.P_post("transaction_list/message", {
					id: this.myid,
					page: this.mypage,
					limit: limit
				}).then(res => {
					if (res.code == 1) {
						this.liuList = res.data;
						this.mypage++
					} else {
						uni.showToast({
							title: `${res.msg},请检查`,
