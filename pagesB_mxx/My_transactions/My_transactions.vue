<template>
	...
	<view class="content">
		<topbar bg_color="#fff" text_color="#fff" :have_top="true">
			...
			<text class=" fs18 pl35 fwb">我卖出的</text>
			<text class="pr5 fs14 fwb" @click='jump' data-url="../My_transaction/My_transaction"
				:data-type='3'>我买到的 </text>
		</topbar>
				...
				<view class="shuju pl15 pr15 pb15 pt10 mt12" v-for="(item,index) in datas" :key="index">
					...
					<view class="bian   ">
						...
						<view class="flex   pr10 pt10">
							...
							<view class="cf yig fs11 ml8 flex aic ju_c">
								一对一过户
							</view>
						</view>
						<view class="flex flex-between pt15">
							<view class="flex aic" style="color:#F43B31;">
								<text class="fs18 fwb">19.00</text>
								<text class="fs12">万</text>
							</view>
							<!-- <view class="fu flex aic ju_c fs13"v-if="item.buy_type==1" @click.stop="jump"
								:data-url="`/pagesA_lzc/pay/pay?id=${item.id}`">
								去付款
							</view> -->
							<view class="fu flex aic ju_c fs13" @click.stop="shouhuo(item.code)" v-if="item.buy_type==3 || item.buy_type==2">
								确认完成
							</view>
							<view class="fu flex aic ju_c fs13" v-if="item.buy_type==4" @click='jump'
								:data-url="`../evaluate/evaluate?id=${item.id}&type=交易`" :data-shifou='true'>
								去评价
							</view>
						</view>
					</view>
